  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (path === '/api/speech' && req.method === 'POST') {
        ...
        const audio = await ai(10, async () => (await upstream('audio/speech', { model: 'gpt-4o-mini-tts', voice: 'coral', input: message.content.slice(0,1500), instructions: 'Speak in Russian in a calm, warm, natural feminine voice.', response_format: 'mp3' })).arrayBuffer());
